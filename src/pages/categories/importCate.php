...
if (isset($_REQUEST['import-excel'])) {
    ...
    if($extension == 'xlsx' || $extension =='xls' || $extension=='csv'){
        ...
        foreach($data as $row){
          ...
          $insert_query = mysqli_query($conn, "insert into categories set 
          categoryName='$categoryName', description='$description', status='$status'");
          if ($insert_query) {
  ...
  <body id="<?php echo $id?>" class="dark:bg-slate-900 ">
    ...
    <div class="sm:ml-64 ">
      ...
      <div class="flex justify-between dark:text-white p-4">
        <div class="text-2xl"> Import Categories </div>
        <a href="categories.php" class=" bg-green-500 p-2"> Back </a>
      </div>
     
     
        <form method="post" enctype="multipart/form-data" class=" flex flex-col gap-4 max-w-lg mx-auto">
            <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white" for="user_avatar">Upload  Excel file</label>
