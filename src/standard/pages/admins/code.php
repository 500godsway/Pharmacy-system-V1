...
// Check if the details already exist ------------
    ...
    $result = insert('pharprofile',$data);
    if($result) {
     redirect('../profiles/profileSettings.php', 'Info added Successifully');
        }else {
...
if(isset($_POST['saveQunitities'])) {
    ...
    require"../../../config/log.php";
    $activityLog->setAction($_SESSION['loggedInUser']['id'], "Added Minimum Quantities $name");
    ...
    $Qty = mysqli_query($conn, "SELECT * FROM minquantities ");
        ...
        $result = insert('minquantities',$data);
        if($result) {
